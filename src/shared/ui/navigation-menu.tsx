...
function NavigationMenuTrigger({
  ...
  return (
    <NavigationMenuPrimitive.Trigger
      ...
      className={cn(
        'group relative inline-flex w-max items-center justify-center rounded-lg px-4 py-2.5 text-sm font-semibold transition-all duration-200 outline-none disabled:pointer-events-none disabled:opacity-50',
        'text-gray-700 hover:text-gray-900 hover:bg-gray-50/80 hover:shadow-sm',
        'focus:bg-gray-50 focus:text-gray-900 focus-visible:ring-2 focus-visible:ring-blue-500/20 focus-visible:ring-offset-1',
        'data-[state=open]:bg-gray-100 data-[state=open]:text-gray-900 data-[state=open]:shadow-sm',
        'data-[state=open]:hover:bg-gray-100 data-[state=open]:focus:bg-gray-100',
        'border border-transparent hover:border-gray-200/50 data-[state=open]:border-gray-200',
        className
      )}
      {...props}
    >
      {children}{' '}
      <ChevronDownIcon
        className='relative top-[1px] ml-1.5 size-3 transition-all duration-200 group-data-[state=open]:rotate-180 text-gray-500 group-hover:text-gray-700'
        aria-hidden='true'
      />
    </NavigationMenuPrimitive.Trigger>
  );
...
function NavigationMenuLink({
  ...
  return (
    <NavigationMenuPrimitive.Link
      ...
      className={cn(
        'flex flex-col justify-center gap-1.5 rounded-lg px-4 py-3 text-sm transition-all duration-200 outline-none',
        'text-gray-700 hover:text-gray-900 hover:bg-gray-50 hover:shadow-sm',
        'focus:bg-gray-50 focus:text-gray-900 focus-visible:ring-2 focus-visible:ring-blue-500/20 focus-visible:ring-offset-1',
        'data-[active=true]:bg-blue-50 data-[active=true]:text-blue-900 data-[active=true]:border-blue-200',
        'data-[active=true]:hover:bg-blue-50 data-[active=true]:focus:bg-blue-50',
        'border border-transparent hover:border-gray-200/50',
        "[&_svg:not([class*='text-'])]:text-gray-500 [&_svg:not([class*='size-'])]:size-4",
        "group-hover:[&_svg:not([class*='text-'])]:text-gray-700",
        className
      )}
      {...props}
    />
    ...
    >
      <div className='flex items-center justify-between px-5 py-4'>
        <div className='space-y-1'>
          ...
          {link.description && (
            <p className='text-muted-foreground line-clamp-1 text-xs'>{link.description}</p>
          )}
        </div>

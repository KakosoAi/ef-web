...
interface InputOTPProps
  extends Omit<React.ComponentPropsWithoutRef<typeof OTPInput>, 'className' | 'maxLength' | 'size' | 'render'>,
...
const InputOTP = React.forwardRef<React.ElementRef<typeof OTPInput>, InputOTPProps>(
  ({ className, containerClassName, variant, size, disabled, maxLength, ...props }, ref) => {
    ...
    return (
      <OTPInput
        ...
        style={{
          fontFamily: otpTheme.typography.fontFamily.sans.join(', '),
        }}
        {...props}
      >
        {props.children}
    ...
    return (
      <div
        ...
        style={{
          ...
          fontSize:
            size === 'sm'
              ? otpTheme.typography.fontSize.xs
              : size === 'lg'
                ? otpTheme.typography.fontSize.base
...
interface InputOTPWithLabelProps extends Omit<InputOTPProps, 'error' | 'success' | 'size'> {
  label?: string;
  description?: string;
  error?: string;
  success?: string;
  ...
  variant?: VariantProps<typeof inputOTPVariants>['variant'];
  size?: VariantProps<typeof inputOTPVariants>['size'];
}

const InputOTPWithLabel = React.forwardRef<
  React.ElementRef<typeof InputOTP>,
  InputOTPWithLabelProps
>(
  ({ label, description, error, success, required, children, className, ...props }, ref) => {
    const otpTheme = useInputOTPTheme();
    ...
    return (
      <div className='space-y-2'>
        ...
        {children ? (
          React.cloneElement(children as React.ReactElement, { ref, maxLength: props.maxLength || 6, onChange: props.onChange, ...props })
        ) : (
          <InputOTP 
            ref={ref} 
...
const InputOTPWithActions = React.forwardRef<
  React.ElementRef<typeof InputOTP>,
  InputOTPWithActionsProps
>(
  (
    {
      showToggle = false,

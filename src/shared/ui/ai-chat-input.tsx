...
const SendButton = memo(
  ({ isDisabled, textColor, isProcessing }: SendButtonProps & { isProcessing?: boolean }) => {
    return (
      <button
        type='submit'
  ...
  ({ glowIntensity, mousePosition, animationDuration, enabled }: GlowEffectsProps) => {
    ...
    return (
      <>
        ...
        <div
          className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity duration-${animationDuration} pointer-events-none`}
          style={{
            boxShadow: `
        ...
        <div
          className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-${animationDuration} pointer-events-none`}
          style={{
            boxShadow: `
        ...
        <div
          className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-25 transition-opacity duration-${animationDuration} bg-gradient-to-r from-transparent via-white/4 to-transparent animate-pulse blur-sm`}
        ></div>
...
const InputArea = memo(
  ({
    ...
    // Keep textarea as single line - no auto-resize
    useEffect(() => {
      if (textareaRef.current) {
        textareaRef.current.style.height = '32px'; // Fixed height for single line
      }
    }, []);
    ...
    return (
      <div className='flex-1 relative h-8 flex items-center'>
        <textarea
          ...
          rows={1}
          className={`w-full h-8 bg-transparent text-sm font-normal text-left self-center placeholder-[#6B7280] border-0 outline-none px-3 pr-10 py-1 z-20 relative resize-none overflow-hidden`}
          style={{
            fontFamily: '"Inter", sans-serif',
  ...
  ({
    ...
    <div className='relative' ref={menuRef}>
      <button
        ...
        className={`h-8 w-8 flex items-center justify-center rounded-full hover:opacity-80 transition-all ml-1 mr-1`}
        style={{
          backgroundColor: `${textColor}10`,
          color: textColor,
        }}
...
export default function AIChatInput({
  ...
  const isSubmitDisabled = disabled || !value.trim() || isProcessing;

  // Calculate width classes - expand to full width
  const hasModeSelected = selectedOptions.length > 0;
  const shouldExpandOnFocus = expandOnFocus && !hasModeSelected;
  const baseWidthClass = 'w-full'; // Always use full width
  const focusWidthClass = ''; // No focus width change needed
    ...
    <ChatInputContext.Provider value={contextValue}>
      <div className='w-full'>
        <div
          ...
          onClick={handleClick}
          className={`relative flex flex-col ${baseWidthClass} ${focusWidthClass} min-h-12 ${backgroundClass} backdrop-blur-xl shadow-lg rounded-3xl p-2 overflow-visible group transition-all duration-${animationDuration}`}
          style={{
